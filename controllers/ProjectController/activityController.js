...
const createActivity = async (req, res) => {
  const uuid = uuidv4();
  var { name, activity_status, start_date, end_date, projectmembers } =
    req.body;
  ...
  const existnigProject = await Project.findOne({
    where: { project_id: project_id },
  });
  if (!existnigProject) {
    return res
      .status(400)
      .json({ message: "we can't get the project to create activity for it" });
  }
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "New activity created", activity: activity });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Server error" });
  }
