...
const handleNewSector = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "New sector created", sector: sector });
  } catch (error) {
    console.error(error);
...
const handleGetAllSectors = async (req, res) => {
  try {
    ...
    const sectorsWithDetails = [];

    for (const sector of sectors) {
      // Fetch  leader for the current sector
      const leader = await User.findAll({
        where: { user_id: sector.leader_id },
      });

      sectorsWithDetails.push({
        sector: sector,
        leader: leader,
      });
    }

    return res.status(200).json(sectorsWithDetails);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Server error" });
  }
